...
def h1(state: list, goal: list) -> int:
    """
    Эвристическая функция h1 — количество неверно расположенных плиток.
    :param state: Текущее состояние.
    :param goal: Целевое состояние.
    :return: Количество фишек, которые не на своих местах.
    """
    return sum(1 for i in range(3) for j in range(3) if state[i][j] != goal[i][j] and state[i][j] != 0)

def h2(state: list, goal: list) -> int:
    """
    Эвристическая функция h2 — манхэттенское расстояние.
    :param state: Текущее состояние.
    :param goal: Целевое состояние.
    :return: Сумма расстояний для каждой фишки от её текущего положения до целевого.
    """
    ...
    for i in range(3):
        for j in range(3):
            if state[i][j] != 0:
                target_i, target_j = goal_positions[state[i][j]]
                distance += abs(i - target_i) + abs(j - target_j)
    return distance

def get_initial_state(new_table) -> list:
    """
    ...
    if new_table != [0]:
        return [list(map(int, str(num))) for num in new_table]
    else:
        return [
            [3, 6, 4],
            [2, 5, 8],
...
def get_followers(current_state: list) -> dict[tuple, list]:
    """
